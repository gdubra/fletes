...
<?php $view['slots']->start('_assets') ?>        
        <?php foreach ($view['assetic']->javascripts(array('bundles/fletes/js/oferta-ctrl.js'),array(),
    ...
    <div class="row">
        <form role="form " name="form" class="col-xs-12 col-sm-6"  data-ng-submit="submit()" novalidate>
            <div class="row no-side-margins form-group">
                ...
                <input type="text" id="oOrigen" name="oOrigen" data-ng-model="oferta.origen" data-ng-model-options="{ debounce: 500 }" placeholder="Localidad origien" typeahead="localidad as localidad.formatted_address for localidad in cosnultar_localidad($viewValue)" typeahead-loading="loadingLocations" class="form-control" typeahead-on-select="marcador_origen()" autocomplete="off" required>
            ...
            <div class="row no-side-margins form-group">
                ...
                <input type="text" data-ng-model="oferta.destino" name="oDestino" placeholder="Localidad destino" data-ng-model-options="{ debounce: 500 }" typeahead="localidad as localidad.formatted_address for localidad in cosnultar_localidad($viewValue)" typeahead-loading="loadingLocations" class="form-control" typeahead-on-select="marcador_destino()" autocomplete="off" required>
            ...
            <div class="row no-side-margins form-group" data-ng-show="oferta.otrasLocalidades.length>=1">
                ...
                <div class="row no-side-margins" data-ng-repeat="localidad in oferta.otrasLocalidades track by $index">
                    <div class="col-xs-10 no-left-padding">
                        <input  type="text" data-ng-model="localidad" placeholder="Ingresar localidad" typeahead="localidad as localidad.formatted_address for localidad in cosnultar_localidad($viewValue)" typeahead-loading="loadingLocations" class="form-control mtop-5" typeahead-on-select="marcador_intermedio(localidad,$index)" autocomplete="off">
                    </div>
            ...
            <div data-ng-hide="oferta.otrasLocalidades.length>=1 && !oferta.otrasLocalidades[oferta.otrasLocalidades.length-1].idKey" class="row no-side-margins form-group">
        ...
        <ui-gmap-google-map class="col-xs-12 col-sm-6 col-md-6" center="mapConfig.center" zoom="mapConfig.zoom" bounds="bounds" control="map.control" options="mapConfig.options">
            <ui-gmap-marker data-ng-if="oferta.origen.idKey" coords="oferta.origen.coords" idKey="oferta.origen.idKey"></ui-gmap-marker>
            <ui-gmap-marker data-ng-if="oferta.destino.idKey" coords="oferta.destino.coords" idKey="oferta.destino.idKey"></ui-gmap-marker>
            <ui-gmap-marker data-ng-repeat="localidad in oferta.otrasLocalidades" data-ng-if="oferta.otrasLocalidades" models="localidad" coords="localidad.coords" idKey="localidad.idKey"></ui-gmap-markers>
        </ui-gmap-google-map>
    </div>
</div>
